...
void write_block_bitmap(int fd)
{
	...
	// TODO It's all yours
	u8 map_value1[BLOCK_SIZE] = {0};
	const int NUM_BITS = BLOCK_SIZE * 8;
    map_value1[0] |= (1 << 0);
    assert((map_value1[0] & (1 << 0)) != 0);
    for (int i = SUPERBLOCK_BLOCKNO; i < LAST_BLOCK; i++) {
        int byte_offset = i / 8;
        int bit_offset = i % 8;
        map_value1[byte_offset] |= (1 << bit_offset);
        assert((map_value1[byte_offset] & (1 << bit_offset)) != 0);
    }
    for (int i = NUM_BLOCKS - 1; i < NUM_BITS; i++) {
        int byte_offset = i / 8;
        int bit_offset = i % 8;
        map_value1[byte_offset] |= (1 << bit_offset);
        assert((map_value1[byte_offset] & (1 << bit_offset)) != 0);
	...
	if (write(fd, map_value1, BLOCK_SIZE) != BLOCK_SIZE)
	{
		errno_exit("write");
	}
}
...
void write_inode_bitmap(int fd)
{
	...
	// TODO It's all yours
	u8 map_value2[BLOCK_SIZE] = {0};
    memset(map_value2, 0xFF, BLOCK_SIZE);

    for (int i = 14; i <= 128; i++) {
        int bit_position = i - 1;
        int byte_offset = bit_position / 8;
        int bit_offset = bit_position % 8;
        map_value2[byte_offset] &= ~(1 << bit_offset);  
        assert((map_value2[byte_offset] & (1 << bit_offset)) == 0);
    }

	if (write(fd, map_value2, BLOCK_SIZE) != BLOCK_SIZE)
	{
		errno_exit("write");
	}
}
...
void write_inode_table(int fd) {
    ...
    struct ext2_inode inode_root = {0};
    inode_root.i_mode = EXT2_S_IFDIR
                        | EXT2_S_IRUSR
                        | EXT2_S_IWUSR
                        | EXT2_S_IXUSR
                        | EXT2_S_IRGRP
                        ...
                        | EXT2_S_IXOTH;
    inode_root.i_uid = 0;
    inode_root.i_size = 1024;
    inode_root.i_atime = current_time;
    inode_root.i_ctime = current_time;
    inode_root.i_mtime = current_time;
    inode_root.i_dtime = 0;
    inode_root.i_gid = 0;
    inode_root.i_links_count = 3;
    inode_root.i_blocks = 2;
    inode_root.i_block[0] = ROOT_DIR_BLOCKNO;
    write_inode(fd, EXT2_ROOT_INO, &inode_root);
    ...
    symlink_inode.i_size = strlen("hello-world");
    ...
    strncpy((char *)symlink_inode.i_block, "hello-world", sizeof(symlink_inode.i_block));
