...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int status = pthread_mutex_init(&hash_table->mutex, NULL);
    if (status != 0) {
        exit(status);  
    }
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	int status = pthread_mutex_lock(&hash_table->mutex);
    if (status != 0) {
        exit(status);  
    }
    ...
    status = pthread_mutex_unlock(&hash_table->mutex);
    if (status != 0) {
        exit(status);  
    }
}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
    ...
    int status = pthread_mutex_destroy(&hash_table->mutex);
    if (status != 0) {
        exit(status); 
    }
